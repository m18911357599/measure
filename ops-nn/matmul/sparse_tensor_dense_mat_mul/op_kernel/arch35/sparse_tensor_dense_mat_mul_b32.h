    ...
    const int32_t p, __gm__ T_IDX* x1Indices, __gm__ T_VAL* x1Values, __gm__ T_VAL* x2, __gm__ T_VAL* y)
{
    ...
    for (int32_t elemIdx = currCoreIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); elemIdx < elemNum;
        ...
        int32_t i, k;
        if constexpr (ADJ_A) {
            i = static_cast<int32_t>(x1Indices[INDICES_DIM_1 * x1VecIdx + 1]); // 转置前x1_indices存的每对元素是[列, 行]
            k = static_cast<int32_t>(x1Indices[INDICES_DIM_1 * x1VecIdx]);
        } else {
            i = static_cast<int32_t>(x1Indices[INDICES_DIM_1 * x1VecIdx]);
            k = static_cast<int32_t>(x1Indices[INDICES_DIM_1 * x1VecIdx + 1]);
        }
        // 读取v1和v2
        T_VAL v1 = x1Values[x1VecIdx];
        T_VAL v2;
        if constexpr (ADJ_B) {
...
private:
    ...
    GlobalTensor<T_VAL> yGm_;       // y的原始位置
    GlobalTensor<T_VAL> yOutputGm_; // y分配到每个核心的数据，偏移后的位置
};
...
template <typename T_IDX, typename T_VAL, bool ADJ_A, bool ADJ_B>
__aicore__ inline void SparseTensorDenseMatMulB32<T_IDX, T_VAL, ADJ_A, ADJ_B>::InitOutputGm()
{
    if (currCoreIdx_ < tilingData_->initAndOutUsedCoreNum) {
        ...
        InitGlobalMemory(yOutputGm_, initEleNumPerCore, static_cast<T_VAL>(0));
    }
    SyncAll();
}

template <typename T_IDX, typename T_VAL, bool ADJ_A, bool ADJ_B>
__aicore__ inline void SparseTensorDenseMatMulB32<T_IDX, T_VAL, ADJ_A, ADJ_B>::Process()
{
