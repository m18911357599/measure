...
/*!
 * \file pad_edge_simt.h
 * \brief pad_edge_simt
 */

#ifndef PAD_EDGE_SIMT_H
#define PAD_EDGE_SIMT_H

#include "pad_common.h"
#include "pad_v3_struct.h"

#ifdef __DAV_FPGA__
constexpr int32_t EDGE_THREAD_DIM = 512;
#else
...
template <typename T>
__aicore__ inline void PadEdgeSimt<T>::Init(
...
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimOne(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, uint32_t outputSize, uint32_t blockIdx,
    uint32_t blockNum, uint32_t inShape0, int32_t left0)
{
    for (uint32_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        inIndex0 = min(max(inIndex0,0), int32_t(inShape0 - 1));
        outputGM[idx] = inputGM[inIndex0];
    }
}

template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimTwo(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, uint32_t outputSize, uint32_t blockIdx,
    uint32_t blockNum, uint32_t outStride0, uint32_t inShape0, uint32_t inShape1, uint32_t m0, uint32_t s0,
    int32_t left0, int32_t left1)
{
    for (uint32_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        inIndex[0] = min(max(inIndex[0],0), int32_t(inShape0 - 1));
        inIndex[1] = min(max(inIndex[1],0), int32_t(inShape1 - 1));
...
template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimThree(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, uint32_t outputSize, uint32_t blockIdx,
    uint32_t blockNum, uint32_t outStride0, uint32_t outStride1, uint32_t inShape0, uint32_t inShape1, uint32_t inShape2,
    uint32_t m0, uint32_t m1, uint32_t s0, uint32_t s1, int32_t left0, int32_t left1, int32_t left2)
{
    for (uint32_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
         idx += blockNum * Simt::GetThreadNum()) {
        uint32_t dstIdx = idx;
        ...
        inIndex[0] = min(max(inIndex[0], 0), int32_t(inShape0 - 1));
        inIndex[1] = min(max(inIndex[1], 0), int32_t(inShape1 - 1));
        inIndex[DIM - 1] = min(max(inIndex[DIM - 1], 0), int32_t(inShape2 - 1));

        uint32_t inputOffset = uint32_t(inIndex[0]) * inShape1 * inShape2 + uint32_t(inIndex[1]) * inShape2 + uint32_t(inIndex[DIM - 1]);
        outputGM[idx] = inputGM[inputOffset];
    }
}

template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimFour(
    ...
    uint32_t blockNum, uint32_t m0, uint32_t m1, uint32_t m2, uint32_t s0, uint32_t s1, uint32_t s2)
{
    ...
    for (uint32_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        for (int32_t i = 0; i < DIM; i++) {
            ...
            inIndex[i] = min(max(inIndex[i], 0), static_cast<int32_t>(tD->inShape[i] - 1));
        }
...
template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimFive(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint32_t outputSize, uint32_t blockIdx,
    uint32_t blockNum, uint32_t m0, uint32_t m1, uint32_t m2, uint32_t m3,
...
template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimSix(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint32_t outputSize, uint32_t blockIdx, uint32_t blockNum,
    uint32_t m0, uint32_t m1, uint32_t m2, uint32_t m3, uint32_t m4, uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3,
    uint32_t s4)
{
    GET_TILING_DATA_PTR_WITH_STRUCT(PadACTilingData, tD, tiling);
    for (uint32_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
...
template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimSeven(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint32_t outputSize, uint32_t blockIdx,
    uint32_t blockNum, uint32_t m0, uint32_t m1, uint32_t m2, uint32_t m3, uint32_t m4, uint32_t m5,
    uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3, uint32_t s4, uint32_t s5)
{
    GET_TILING_DATA_PTR_WITH_STRUCT(PadACTilingData, tD, tiling);
    for (uint32_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
         idx += blockNum * Simt::GetThreadNum()) {
...
template <typename T, int32_t DIM>
__simt_vf__ LAUNCH_BOUND(EDGE_THREAD_DIM) __aicore__ void SimtComputeEdgeDimEight(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint32_t outputSize, uint32_t blockIdx, uint32_t blockNum,
...
template <typename T>
__aicore__ inline void PadEdgeSimt<T>::Process(GM_ADDR tiling)
{
    ...
    if (mDimNum == 1) {
        Simt::VF_CALL<SimtComputeEdgeDimOne<T>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
    ...
    if (mDimNum == 2) {
        Simt::VF_CALL<SimtComputeEdgeDimTwo<T, 2>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
            outputSize, mBlockIdx_, blockNum, mTD_->outStride[0], mTD_->inShape[0], mTD_->inShape[1], m[0],
            s[0], mTD_->leftPad[0], mTD_->leftPad[1]);
    } else if (mDimNum == 3) {
        Simt::VF_CALL<SimtComputeEdgeDimThree<T, 3>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
            outputSize, mBlockIdx_, blockNum, mTD_->outStride[0], mTD_->outStride[1], mTD_->inShape[0],
            mTD_->inShape[1], mTD_->inShape[2], m[0], m[1], s[0], s[1], mTD_->leftPad[0], mTD_->leftPad[1],
            mTD_->leftPad[2]);
    } else if (mDimNum == 4) {
        Simt::VF_CALL<SimtComputeEdgeDimFour<T, 4>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
            tiling, outputSize, mBlockIdx_, blockNum, m[0], m[1], m[2], s[0], s[1], s[2]);
    } else if (mDimNum == 5) {
        Simt::VF_CALL<SimtComputeEdgeDimFive<T, 5>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
            tiling, outputSize, mBlockIdx_, blockNum, m[0], m[1], m[2], m[3], s[0], s[1], s[2], s[3]);
    } else if (mDimNum == 6) {
        Simt::VF_CALL<SimtComputeEdgeDimSix<T, 6>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
            tiling, outputSize, mBlockIdx_, blockNum, m[0], m[1], m[2], m[3], m[4], s[0], s[1], s[2], s[3], s[4]);
    } else if (mDimNum == 7) {
        Simt::VF_CALL<SimtComputeEdgeDimSeven<T, 7>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
            tiling, outputSize, mBlockIdx_, blockNum, m[0], m[1], m[2], m[3], m[4], m[5],
            s[0], s[1], s[2], s[3], s[4], s[5]);
    } else if (mDimNum == 8) {
        Simt::VF_CALL<SimtComputeEdgeDimEight<T, 8>>(
            Simt::Dim3(EDGE_THREAD_DIM), (__gm__ T*)(mInputGM_.GetPhyAddr()), (__gm__ volatile T*)(mOutputGM_.GetPhyAddr()),
