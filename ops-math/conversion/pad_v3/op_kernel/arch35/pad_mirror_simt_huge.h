...
template <typename T, int32_t KEY>
__aicore__ inline void PadReflectSimtHuge<T, KEY>::Init(
...
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_HALF_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimOne(
    ...
    uint64_t inShape0, int64_t left0)
{
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        if constexpr (IS_REFLECT) {
            inIndex0 = abs(inIndex0 - left0) - abs(inIndex0 - (int64_t(inShape0) + left0 - 1)) - inIndex0 + left0 +
                       int64_t(inShape0) - 1;
        } else {
            inIndex0 =
                (abs((inIndex0 - left0) * 2 + 1) - abs((inIndex0 - (int64_t(inShape0) + left0 - 1)) * 2 - 1)) / 2 -
                inIndex0 + left0 + int64_t(inShape0) - 1;
        }
    }
}

template <typename T, int32_t DIM, bool IS_REFLECT>
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_HALF_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimTwo(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, uint64_t outputSize, uint32_t blockIdx, uint32_t blockNum,
    uint64_t outStride0, uint64_t inShape0, uint64_t inShape1, uint64_t m0, uint64_t s0, int64_t left0, int64_t left1)
{
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        if constexpr (IS_REFLECT) {
            inIndex[0] =
                abs(inIndex[0]) - abs(inIndex[0] - (int64_t(inShape0) - 1)) - inIndex[0] + int64_t(inShape0) - 1;
            inIndex[1] =
                abs(inIndex[1]) - abs(inIndex[1] - (int64_t(inShape1) - 1)) - inIndex[1] + int64_t(inShape1) - 1;
        } else {
            inIndex[0] = (abs(inIndex[0] * 2 + 1) - abs((inIndex[0] - (int64_t(inShape0) - 1)) * 2 - 1)) / 2 -
                         inIndex[0] + int64_t(inShape0) - 1;
            inIndex[1] = (abs(inIndex[1] * 2 + 1) - abs((inIndex[1] - (int64_t(inShape1) - 1)) * 2 - 1)) / 2 -
                         inIndex[1] + int64_t(inShape1) - 1;
        }
...
template <typename T, int32_t DIM, bool IS_REFLECT>
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_HALF_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimThree(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, uint64_t outputSize, uint32_t blockIdx, uint32_t blockNum,
    ...
    uint64_t m1, uint64_t s0, uint64_t s1, int64_t left0, int64_t left1, int64_t left2)
{
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        if constexpr (IS_REFLECT) {
            ...
            inIndex[1] =
                abs(inIndex[1]) - abs(inIndex[1] - (int64_t(inShape1) - 1)) - inIndex[1] + int64_t(inShape1) - 1;
            inIndex[DIM - 1] = abs(inIndex[DIM - 1]) - abs(inIndex[DIM - 1] - (int64_t(inShape2) - 1)) -
                               inIndex[DIM - 1] + int64_t(inShape2) - 1;
        } else {
            inIndex[0] = (abs(inIndex[0] * 2 + 1) - abs((inIndex[0] - (int64_t(inShape0) - 1)) * 2 - 1)) / 2 -
                         inIndex[0] + int64_t(inShape0) - 1;
            inIndex[1] = (abs(inIndex[1] * 2 + 1) - abs((inIndex[1] - (int64_t(inShape1) - 1)) * 2 - 1)) / 2 -
                         inIndex[1] + int64_t(inShape1) - 1;
            inIndex[DIM - 1] =
                (abs(inIndex[DIM - 1] * 2 + 1) - abs((inIndex[DIM - 1] - (int64_t(inShape2) - 1)) * 2 - 1)) / 2 -
                inIndex[DIM - 1] + int64_t(inShape2) - 1;
        }
...
template <typename T, int32_t DIM, bool IS_REFLECT>
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_QUATER_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimFour(
    ...
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        if constexpr (IS_REFLECT) {
            for (int32_t i = 0; i < DIM; i++) {
                inIndex[i] -= static_cast<int64_t>(tD->leftPad[i]);
                inIndex[i] = abs(inIndex[i]) - abs(inIndex[i] - (static_cast<int64_t>(tD->inShape[i]) - 1)) -
                             inIndex[i] + static_cast<int64_t>(tD->inShape[i]) - 1;
            }
        } else {
            for (int32_t i = 0; i < DIM; i++) {
                inIndex[i] -= static_cast<int64_t>(tD->leftPad[i]);
                inIndex[i] =
                    (abs(inIndex[i] * 2 + 1) - abs((inIndex[i] - (static_cast<int64_t>(tD->inShape[i]) - 1)) * 2 - 1)) /
                        2 -
                    inIndex[i] + static_cast<int64_t>(tD->inShape[i]) - 1;
            }
        }

        uint64_t inputOffset = static_cast<uint64_t>(inIndex[0]) * static_cast<uint64_t>(tD->inStride[0]) +
                               static_cast<uint64_t>(inIndex[1]) * static_cast<uint64_t>(tD->inStride[1]) +
                               static_cast<uint64_t>(inIndex[2]) * static_cast<uint64_t>(tD->inStride[2]) +
...
template <typename T, int32_t DIM, bool IS_REFLECT>
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_QUATER_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimFive(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint64_t outputSize, uint32_t blockIdx,
    ...
    uint64_t s3)
{
    ...
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        uint64_t inputOffset = static_cast<uint64_t>(inIndex[0]) * static_cast<uint64_t>(tD->inStride[0]) +
                               static_cast<uint64_t>(inIndex[1]) * static_cast<uint64_t>(tD->inStride[1]) +
                               static_cast<uint64_t>(inIndex[2]) * static_cast<uint64_t>(tD->inStride[2]) +
                               static_cast<uint64_t>(inIndex[3]) * static_cast<uint64_t>(tD->inStride[3]) +
                               static_cast<uint64_t>(inIndex[DIM - 1]);
        outputGM[idx] = inputGM[inputOffset];
    }
}
...
template <typename T, int32_t DIM, bool IS_REFLECT>
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_QUATER_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimSix(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint64_t outputSize, uint32_t blockIdx,
    ...
    uint64_t s2, uint64_t s3, uint64_t s4)
{
    ...
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        uint64_t inputOffset = static_cast<uint64_t>(inIndex[0]) * static_cast<uint64_t>(tD->inStride[0]) +
                               ...
                               static_cast<uint64_t>(inIndex[2]) * static_cast<uint64_t>(tD->inStride[2]) +
                               static_cast<uint64_t>(inIndex[3]) * static_cast<uint64_t>(tD->inStride[3]) +
                               static_cast<uint64_t>(inIndex[4]) * static_cast<uint64_t>(tD->inStride[4]) +
                               static_cast<uint64_t>(inIndex[DIM - 1]);
        outputGM[idx] = inputGM[inputOffset];
    }
}
...
template <typename T, int32_t DIM, bool IS_REFLECT>
__simt_vf__ LAUNCH_BOUND(REFLECT_HUGE_QUATER_THREAD_DIM) __aicore__ void SimtComputeReflectHugeDimSeven(
    __gm__ T* inputGM, __gm__ volatile T* outputGM, GM_ADDR tiling, uint64_t outputSize, uint32_t blockIdx,
    ...
    uint64_t s1, uint64_t s2, uint64_t s3, uint64_t s4, uint64_t s5)
{
    ...
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        uint64_t inputOffset = static_cast<uint64_t>(inIndex[0]) * static_cast<uint64_t>(tD->inStride[0]) +
                               ...
                               static_cast<uint64_t>(inIndex[3]) * static_cast<uint64_t>(tD->inStride[3]) +
                               static_cast<uint64_t>(inIndex[4]) * static_cast<uint64_t>(tD->inStride[4]) +
                               static_cast<uint64_t>(inIndex[5]) * static_cast<uint64_t>(tD->inStride[5]) +
                               static_cast<uint64_t>(inIndex[DIM - 1]);
        outputGM[idx] = inputGM[inputOffset];
    }
}
    ...
    uint64_t s0, uint64_t s1, uint64_t s2, uint64_t s3, uint64_t s4, uint64_t s5, uint64_t s6)
{
    ...
    for (uint64_t idx = blockIdx * Simt::GetThreadNum() + Simt::GetThreadIdx(); idx < outputSize;
        ...
        uint64_t inputOffset = static_cast<uint64_t>(inIndex[0]) * static_cast<uint64_t>(tD->inStride[0]) +
                               ...
                               static_cast<uint64_t>(inIndex[4]) * static_cast<uint64_t>(tD->inStride[4]) +
                               static_cast<uint64_t>(inIndex[5]) * static_cast<uint64_t>(tD->inStride[5]) +
                               static_cast<uint64_t>(inIndex[6]) * static_cast<uint64_t>(tD->inStride[6]) +
                               static_cast<uint64_t>(inIndex[DIM - 1]);
        outputGM[idx] = inputGM[inputOffset];
    }
}
