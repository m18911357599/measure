...
/*!
 * \file view_copy_dim2.h
 * \brief
 */

#ifndef VIEW_COPY_DIM2_H_
#define VIEW_COPY_DIM2_H_
...
template <typename T>
class ViewCopyDim2 : public ViewCopyBase<T> {
public:
    __aicore__ inline ViewCopyDim2(TPipe &pipe, const ViewCopyTilingData *tilingData) :
...
template <typename T>
__aicore__ inline void ViewCopyDim2<T>::Init(
    GM_ADDR dst, GM_ADDR dstSize, GM_ADDR dstStride, GM_ADDR dstStorageOffset,
...
template <typename T>
__aicore__ inline void ViewCopyDim2<T>::Process()
{
    ...
    const DataCopyExtParams &copyParams)
{
    ...
    if (tilingData_->enableMovAlign != 0) {
        ...
        copyParam.blockCount = loopInfo.loopSize[DIM1];
        copyParam.blockLen = loopInfo.loopSize[DIM0] * sizeof(T);
        copyParam.srcStride = (loopInfo.loopSrcStride[DIM1] - loopInfo.loopSize[DIM0]) * sizeof(T);
        copyParam.dstStride = (loopInfo.loopDstStride[DIM1] -
...
template <typename T>
__aicore__ inline void ViewCopyDim2<T>::InitCopyParams()
{
    dmaParam_ = {
        {
            {   // src stride
            ...
            {ZERO_U8, ZERO_U8},  // left pad
            {ZERO_U8, ZERO_U8}   // right pad
        },
        0   //pad value
    };
